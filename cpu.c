...
static inline void 
_branch(CPU* cpu, int flag, int check, uint16_t address)
{
    if (flag == check) {
        cpu->cycles += 1;
...
static inline void 
_stack_push(CPU* cpu, uint8_t value)
{
...
static uint16_t _addr_ABS(CPU* cpu)
{
...
static uint16_t _addr_ABS_X(CPU* cpu)
{
    uint8_t addr_lo = _read_PC_incr(cpu);
    uint8_t addr_hi = _read_PC_incr(cpu);
...
static uint16_t _addr_ABS_Y(CPU* cpu)
{
    uint8_t addr_lo = _read_PC_incr(cpu);
    uint8_t addr_hi = _read_PC_incr(cpu);
    uint16_t base   = (uint16_t)((addr_hi << 8) | addr_lo);
...
static uint16_t _addr_ACC(CPU* cpu)
{
    (void)cpu; // not used
    return 0xFFFF;  // 0xFFFF => "use A register directly"
}

static uint16_t _addr_IMP(CPU* cpu)
{
    (void)cpu; 
    return 0xFFFF;  // implied
}

static uint16_t _addr_IMM(CPU* cpu)
{
...
static uint16_t _addr_IND(CPU* cpu)
{
    uint8_t addr_lo = _read_PC_incr(cpu);
    uint8_t addr_hi = _read_PC_incr(cpu);
...
static uint16_t _addr_IND_X(CPU* cpu)
{
    uint8_t addr_lo = _read_PC_incr(cpu);
...
static uint16_t _addr_IND_Y(CPU* cpu)
{
    uint8_t addr_lo = _read_PC_incr(cpu);
...
static uint16_t _addr_REL(CPU* cpu)
{
    // signed offset
...
static uint16_t _addr_ZP(CPU* cpu)
{
    return _read_PC_incr(cpu);
}

static uint16_t _addr_ZP_X(CPU* cpu)
{
    uint8_t addr_lo = _read_PC_incr(cpu);
    return (uint8_t)(addr_lo + cpu->X);
}

static uint16_t _addr_ZP_Y(CPU* cpu)
{
    uint8_t addr_lo = _read_PC_incr(cpu);
...
static void _instr_ADC(CPU* cpu, uint16_t address) {
...
static void _instr_AND(CPU* cpu, uint16_t address) {
    uint8_t val = cpu_read(cpu->bus, address);
...
static void _instr_ASL(CPU* cpu, uint16_t address) {
...
static void _instr_BCC(CPU* cpu, uint16_t address) {
    _branch(cpu, cpu->P.C, 0, address);
}

static void _instr_BCS(CPU* cpu, uint16_t address) {
    _branch(cpu, cpu->P.C, 1, address);
}

static void _instr_BEQ(CPU* cpu, uint16_t address) {
    _branch(cpu, cpu->P.Z, 1, address);
}

static void _instr_BIT(CPU* cpu, uint16_t address) {
    uint8_t val = cpu_read(cpu->bus, address);
    uint8_t result = cpu->A & val;
    cpu->P.Z = (result == 0);
    cpu->P.V = (val & 0x40) >> 6;
...
static void _instr_BMI(CPU* cpu, uint16_t address) {
    _branch(cpu, cpu->P.N, 1, address);
}

static void _instr_BNE(CPU* cpu, uint16_t address) {
    _branch(cpu, cpu->P.Z, 0, address);
}

static void _instr_BPL(CPU* cpu, uint16_t address) {
    _branch(cpu, cpu->P.N, 0, address);
}

static void _instr_BRK(CPU* cpu, uint16_t address) {
...
static void _instr_BVC(CPU* cpu, uint16_t address) {
    _branch(cpu, cpu->P.V, 0, address);
}

static void _instr_BVS(CPU* cpu, uint16_t address) {
    _branch(cpu, cpu->P.V, 1, address);
}

static void _instr_CLC(CPU* cpu, uint16_t address) {
    cpu->P.C = 0;
}

static void _instr_CLD(CPU* cpu, uint16_t address) {
    cpu->P.D = 0;
}

static void _instr_CLI(CPU* cpu, uint16_t address) {
    cpu->P.I = 0;
}

static void _instr_CLV(CPU* cpu, uint16_t address) {
    cpu->P.V = 0;
}

static void _instr_CMP(CPU* cpu, uint16_t address) {
    uint8_t val = cpu_read(cpu->bus, address);
...
static void _instr_CPX(CPU* cpu, uint16_t address) {
    uint8_t val = cpu_read(cpu->bus, address);
...
static void _instr_CPY(CPU* cpu, uint16_t address) {
    uint8_t val = cpu_read(cpu->bus, address);
...
static void _instr_DEC(CPU* cpu, uint16_t address) {
    uint8_t val = cpu_read(cpu->bus, address);
    uint8_t result = val - 1;
    cpu->P.Z = (result == 0);
    cpu->P.N = (result & 0x80) >> 7;
...
static void _instr_DEX(CPU* cpu, uint16_t address) {
    cpu->X -= 1;
    cpu->P.Z = (cpu->X == 0);
    cpu->P.N = (cpu->X & 0x80) >> 7;
}

static void _instr_DEY(CPU* cpu, uint16_t address) {
    cpu->Y -= 1;
    cpu->P.Z = (cpu->Y == 0);
    cpu->P.N = (cpu->Y & 0x80) >> 7;
}

static void _instr_EOR(CPU* cpu, uint16_t address) {
    uint8_t val = cpu_read(cpu->bus, address);
    uint8_t result = cpu->A ^ val;
    cpu->P.Z = (result == 0);
    cpu->P.N = (result & 0x80) >> 7;
...
static void _instr_INC(CPU* cpu, uint16_t address) {
    uint8_t val = cpu_read(cpu->bus, address);
    uint8_t result = val + 1;
    cpu->P.Z = (result == 0);
    cpu->P.N = (result & 0x80) >> 7;
...
static void _instr_INX(CPU* cpu, uint16_t address) {
    cpu->X += 1;
    cpu->P.Z = (cpu->X == 0);
    cpu->P.N = (cpu->X & 0x80) >> 7;
}

static void _instr_INY(CPU* cpu, uint16_t address) {
    cpu->Y += 1;
    cpu->P.Z = (cpu->Y == 0);
    cpu->P.N = (cpu->Y & 0x80) >> 7;
}

static void _instr_JMP(CPU* cpu, uint16_t address) {
    cpu->PC = address;
}

static void _instr_JSR(CPU* cpu, uint16_t address) {
...
static void _instr_LDA(CPU* cpu, uint16_t address) {
    uint8_t val = cpu_read(cpu->bus, address);
    cpu->P.Z = (val == 0);
    cpu->P.N = (val & 0x80) >> 7;
    cpu->A = val;
...
static void _instr_LDX(CPU* cpu, uint16_t address) {
    uint8_t val = cpu_read(cpu->bus, address);
    cpu->P.Z = (val == 0);
    cpu->P.N = (val & 0x80) >> 7;
    cpu->X = val;
...
static void _instr_LDY(CPU* cpu, uint16_t address) {
    uint8_t val = cpu_read(cpu->bus, address);
    cpu->P.Z = (val == 0);
    cpu->P.N = (val & 0x80) >> 7;
    cpu->Y = val;
...
static void _instr_LSR(CPU* cpu, uint16_t address) {
    uint8_t val = address == 0xFFFF ? cpu->A : cpu_read(cpu->bus, address);
    uint8_t result = val >> 1;
    cpu->P.C = val & 1;
    cpu->P.Z = (result == 0);
...
static void _instr_NOP(CPU* cpu, uint16_t address) {
    return;
}

static void _instr_ORA(CPU* cpu, uint16_t address) {
    uint8_t val = cpu_read(cpu->bus, address);
    uint8_t result = cpu->A | val;
    cpu->P.Z = (result == 0);
    cpu->P.N = (result & 0x80) >> 7;
...
static void _instr_PHA(CPU* cpu, uint16_t address) {
    _stack_push(cpu, cpu->A);
}

static void _instr_PHP(CPU* cpu, uint16_t address) {
    _stack_push(cpu, cpu->P.value);
}

static void _instr_PLA(CPU* cpu, uint16_t address) {
...
static void _instr_PLP(CPU* cpu, uint16_t address) {
    cpu->P.value =_stack_pull(cpu);
}

static void _instr_ROL(CPU* cpu, uint16_t address) {
    uint8_t val = address == 0xFFFF ? cpu->A : cpu_read(cpu->bus, address);
...
static void _instr_ROR(CPU* cpu, uint16_t address) {
    uint8_t val = address == 0xFFFF ? cpu->A : cpu_read(cpu->bus, address);
...
static void _instr_RTI(CPU* cpu, uint16_t address) {
    cpu->P.value = _stack_pull(cpu);
...
static void _instr_RTS(CPU* cpu, uint16_t address) {
    uint8_t addr_lo = _stack_pull(cpu);
    uint8_t addr_hi = _stack_pull(cpu);
    uint16_t new_addr = (addr_hi << 8 ) | addr_lo;
    cpu->PC = new_addr + 1;
...
static void _instr_SBC(CPU* cpu, uint16_t address) {
...
static void _instr_SEC(CPU* cpu, uint16_t address) {
    cpu->P.C = 1;
}

static void _instr_SED(CPU* cpu, uint16_t address) {
    cpu->P.D = 1;
}

static void _instr_SEI(CPU* cpu, uint16_t address) {
    cpu->P.I = 1;
}

static void _instr_STA(CPU* cpu, uint16_t address) {
    cpu_write(cpu->bus, address, cpu->A);
}

static void _instr_STX(CPU* cpu, uint16_t address) {
    cpu_write(cpu->bus, address, cpu->X);
}

static void _instr_STY(CPU* cpu, uint16_t address) {
    cpu_write(cpu->bus, address, cpu->Y);
}

static void _instr_TAX(CPU* cpu, uint16_t address) {
    cpu->P.Z = (cpu->A == 0);
    cpu->P.N = (cpu->A & 0x80) >> 7;
    cpu->X = cpu->A;
}

static void _instr_TAY(CPU* cpu, uint16_t address) {
    cpu->P.Z = (cpu->A == 0);
    cpu->P.N = (cpu->A & 0x80) >> 7;
    cpu->Y = cpu->A;
}

static void _instr_TSX(CPU* cpu, uint16_t address) {
    cpu->P.Z = (cpu->SP == 0);
    cpu->P.N = (cpu->SP & 0x80) >> 7;
    cpu->X = cpu->SP;
}

static void _instr_TXA(CPU* cpu, uint16_t address) {
    cpu->P.Z = (cpu->X == 0);
    cpu->P.N = (cpu->X & 0x80) >> 7;
    cpu->A = cpu->X;
}

static void _instr_TXS(CPU* cpu, uint16_t address) {
    cpu->P.Z = (cpu->X == 0);
    cpu->P.N = (cpu->X & 0x80) >> 7;
    cpu->SP = cpu->X;
}

static void _instr_TYA(CPU* cpu, uint16_t address) {
    cpu->P.Z = (cpu->Y == 0);
    cpu->P.N = (cpu->Y & 0x80) >> 7;
    cpu->A = cpu->Y;
}
...
//------------------------------------------------------------------------------
// Public methods (init_opcode_table, cpu_reset, cpu_step, handle_nmi)
//------------------------------------------------------------------------------
